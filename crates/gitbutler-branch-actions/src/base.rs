...
pub mod upstream_integration {
    ...
    pub struct UpstreamIntegrationStatusesContext<'a> {
        _perm: Option<&'a WorktreeReadPermission>,
    ...
    impl<'a> UpstreamIntegrationStatusesContext<'a> {
        pub(crate) fn open(
            command_context: &'a CommandContext,
            perm: &'a WorktreeReadPermission,
        ) -> Result<Self> {
            let target = default_target(&command_context.project().gb_dir())?;
            ...
            let virtual_branches_in_workspace =
                virtual_branches_handle.list_branches_in_workspace()?;
    ...
    pub fn upstream_integration_statuses(
        context: UpstreamIntegrationStatusesContext,
    ) -> Result<BranchStatuses> {
        let UpstreamIntegrationStatusesContext {
            repository,
            new_target,
            old_target,
            virtual_branches_in_workspace,
    ...
    mod test {
        ...
        fn test_up_to_date_if_head_commits_equivalent() {
            ...
            let head_commit =
                commit_file(&repository, Some(&initial_commit), &[("foo.txt", "baz")]);

            let context = UpstreamIntegrationStatusesContext {
                _perm: None,
